...
class UserCreationForm(forms.Form):
    ...
    password2 = forms.CharField(label="Conferma password", widget=forms.PasswordInput(attrs={"class": "form-control"}))
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username__iexact=username).exists():
            raise forms.ValidationError("Esiste già un utente con questo username.")
...
class EditProfileForm(forms.Form):
    ...
    phone_number = forms.CharField(
        required=False,
        widget=forms.TextInput(attrs={"class": "form-control", "inputmode": "tel", "placeholder": "+39 333 123 4567"}),
    )
    profile_picture = forms.ImageField(widget=forms.ClearableFileInput(attrs={"class": "form-control"}), required=False)
    steam_url = forms.URLField(widget=forms.TextInput(attrs={"class": "form-control"}), required=False)
...
class CSVImportForm(forms.Form):
    ...
    import_type = forms.ChoiceField(choices=IMPORT_CHOICES, widget=forms.Select(attrs={"class": "form-select"}))
    csv_file = forms.FileField(widget=forms.ClearableFileInput(attrs={"class": "form-control", "accept": ".csv"}))

...
class UserCreationForm(forms.Form):
    ...
    password2 = forms.CharField(
        label="Conferma password", widget=forms.PasswordInput(attrs={"class": "form-control"})
    )
...
class EditProfileForm(forms.Form):
    ...
    phone_number = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "inputmode": "tel", "placeholder": "+39 333 123 4567"}
        ),
    )
...
class CSVImportForm(forms.Form):
    ...
    import_type = forms.ChoiceField(
        choices=IMPORT_CHOICES, widget=forms.Select(attrs={"class": "form-select"})
    )
